...
def prepare_audio(mean, istd, options):
    ...
    if not options.partial:
        ...
        spects = (simplecache.cached(options.cache_spectra and
                                     ...
                                     os.path.join(datadir, 'audio', fn),
                                     options.save_input, options.dump_path, sample_rate, frame_len, fps, options.offset,
                                     options.duration)
                  for fn in filelist)
    ...
    if (options.transform == 'mel'):
        ...
        filterbank_pinv = np.ones(
            (mel_bands, bin_mel_max))  # dummy of no use in this case. need to do as same code is used to return
...
def _compile_prediction_function_audio(modelfile, input_dim, excerpt_size):
    """
...
if __name__ == '__main__':
    ...
    for excerpt_id, excerpt in enumerate(list_excerpts):
        ...
        explanation, seg = explainer.explain_instance(image=spect, classifier_fn=prediction_fn_audio, hide_color=0,
        ...
        plt.subplot(3, 1, 2)
        librosa.display.specshow(temp.T, y_axis='mel', x_axis='time', sr=22050, hop_length=315, fmin=27.5, fmax=8000,
                                 cmap='coolwarm')
        plt.title('Top-3 interpretable components from SLIME', fontsize=10)
        plt.subplot(3, 1, 3)
        plt.title('Pos. saliency (grd > 0.5)', fontsize=10)
        librosa.display.specshow(ss_d.T, sr=22050, hop_length=315, fmin=27.5, fmax=8000, y_axis='mel', x_axis='time',
                                 cmap='OrRd')
        plt.tight_layout()
        plt.savefig(os.path.join(options.dump_path, 'analysis' + str(excerpt_id).zfill(3) + '.eps'), format='eps',
                    dpi=300)
